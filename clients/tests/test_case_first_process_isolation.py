...
class CaseFirstProcessIsolationTests(TestCase):
    ...
    def test_process_email_logs_are_bound_to_the_supplied_case(self) -> None:
        ...
        self.assertTrue(log.idempotency_key)
    ...
    def test_purpose_context_exposes_case_key_while_preserving_legacy_alias(self) -> None:
        ...
        self.assertEqual(context["original_case_purpose_label"], context["original_client_purpose_label"])
    ...
    def test_manual_document_reminder_logs_one_email_per_case(self) -> None:
        document_a = create_test_document(
            self.client_obj,
            case=self.case_a,
            expiry_date=timezone.localdate() + timedelta(days=2),
        ...
        result = send_document_reminder_for_client(client=self.client_obj, actor=None)

        self.assertFalse(result.email_sent)
        self.assertEqual(result.emails_sent_count, 0)
        logs = list(EmailLog.objects.filter(template_type="expiring_documents").order_by("case_id"))
        self.assertEqual(len(logs), 2)
        self.assertEqual({log.case_id for log in logs}, {self.case_a.pk, self.case_b.pk})
        self.assertEqual(len({log.idempotency_key for log in logs}), 2)
        self.assertTrue(all(log.idempotency_key for log in logs))

    def test_manual_document_reminder_single_case_keeps_single_email_behavior(self) -> None:
        document = create_test_document(
            self.client_obj,
            case=self.case_b,
            expiry_date=timezone.localdate() + timedelta(days=3),
        ...
        def fake_sender(client, documents, *, sent_by=None, case=None):
            calls.append((case.pk if case else None, [doc.pk for doc in documents]))
            return 1
        ...
        self.assertEqual(calls, [(self.case_b.pk, [document.pk])])

    def test_case_form_rejects_non_family_case_with_family_role(self) -> None:
        form = CaseForm(
            data={
                ...
                "application_type": "",
                "basis_of_stay": "",
                "workflow_stage": self.case_a.workflow_stage,
                "submission_date": "",
                "fingerprints_date": "",
                "company": "",
                "version": self.case_a.version,
            },
            instance=self.case_a,
        )

        self.assertFalse(form.is_valid())
        self.assertIn("family_role", form.errors)

    def test_case_form_requires_family_role_for_family_case(self) -> None:
        form = CaseForm(
            data={
                "authority_case_number": "",
                "application_purpose": "family",
                ...
                "application_type": "",
                "basis_of_stay": "",
                "workflow_stage": self.case_b.workflow_stage,
                "submission_date": "",
                "fingerprints_date": "",
                "company": "",
                "version": self.case_b.version,
            },
            instance=self.case_b,
        )
