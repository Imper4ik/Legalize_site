...
class AdminDashboardView(StaffRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["docs_awaiting_confirmation"] = Document.objects.filter(
        ...
        context["upcoming_reminders"] = Reminder.objects.filter(
            is_active=True,
            due_date__gt=today,
        ).count()
        ...
        context["total_revenue"] = sum(
            p.amount_paid for p in Payment.objects.filter(status="paid")
        )

        # --- Email Campaigns ---
        context["failed_campaigns"] = EmailCampaign.objects.filter(
