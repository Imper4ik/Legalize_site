...
- «Базовый URL inPOL» — тот же адрес, что и в браузере (например, `https://inpol.mazowieckie.pl`). Не добавляйте
  в конец `/login`: клиент сам обращается к нужным API и вернёт 403, если базовый адрес оканчивается на `/login`.
...
- При запуске контейнера скрипт `start.sh` поднимает фоновую задачу, которая каждые `INPOL_POLL_INTERVAL` секунд (по умолчанию 900, то есть 15 минут) вызывает `python manage.py check_inpol --silent` и синхронизирует статусы для всех клиентов.
- Чтобы отключить автообновление, выставьте `INPOL_AUTO_POLL=0`. Интервал можно менять через `INPOL_POLL_INTERVAL` (в секундах).
- На карточке клиента отображается последний статус inPOL и кнопка «Проверить статус сейчас», которая делает точечный запрос только для этого клиента, используя активную учётку.
...
2. Зайдите в `/admin/clients/inpolaccount/` и добавьте активную учётку inPOL (или заполните `.env` переменные из разделов выше).
   Альтернатива: войдите как сотрудник и откройте раздел **inPOL** в верхнем меню сайта (ведёт на `/clients/inpol/account/`) — там тоже можно сохранить активную учётку.
